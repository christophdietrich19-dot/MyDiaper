...
Die Web-App bleibt direkt über `index.html` nutzbar. Zusätzlich ist sie für eine Capacitor-Hülle vorbereitet. `capacitor.config.json` verwendet `de.christophit.mydiaper`, `MyDiaper Testversion 1.1.5` und `www` als Web-Ausgabe.
...
- `npm run android:build:test-release` erzeugt zusätzlich eine gehärtete, R8-optimierte und dauerhaft signierte APK für den vertrauten Testerkreis. Der erfolgreiche Build `MyDiaper-Testversion-1.1.5.apk` vom 15.09.2026 ist nicht debug-fähig, schließt App-Daten aus Backups/Geräteübertragung aus, verbietet Klartextverkehr und wurde mit APK Signature Scheme v2/v3 verifiziert. SHA-256: `d9bdb55775c5dce62aba9c5c408ad1cc42e7ffe98130cee59cb6e5b70faba5a7`.
...
- Paketname: `de.christophit.mydiaper`; Versioncode `7`, Versionsname `1.1.5-test`; `minSdk 24`, `compileSdk 36`, `targetSdk 36`.
