...
(function(){
  ...
  const ui = { route:'today', offerMode:'local', marketFilter:'Alle', marketQuery:'', marketOnlyMine:false, offerView:'map' };
  ...
  function closeModal(){ modalRoot.className='modal-root'; modalRoot.innerHTML='';if(modalReturnFocus&&typeof modalReturnFocus.focus==='function')modalReturnFocus.focus();modalReturnFocus=null; }
  function modal(title,body){ modalReturnFocus=document.activeElement||null;modalRoot.className='modal-root open'; modalRoot.innerHTML=`<div class="modal" role="dialog" aria-modal="true" aria-label="${esc(title)}"><span class="modal-grabber"></span><div class="modal-header"><h2>${esc(title)}</h2><button class="icon-btn" data-action="close-modal" aria-label="Dialog schließen">${icon('close')}</button></div>${body}</div>`;setTimeout(()=>{const target=modalRoot.querySelector&&modalRoot.querySelector('input, select, textarea, button');if(target&&typeof target.focus==='function')target.focus();},0); }
  ...
  function setFormModal(existing){
    ...
    modal(existing?'Windelset bearbeiten':'Windelset hinzufügen',`<form id="setForm" class="stack" data-child-id="${esc(c.id)}" data-id="${esc(existing?.id||'')}"><div class="input-group"><label for="setLabel">Art / Bezeichnung</label><select class="select" id="setLabel" name="label">${['Windel','Pants','Nachtwindel','Schwimmwindel'].map(label=>`<option ${label===set.label?'selected':''}>${label}</option>`).join('')}</select></div><div class="form-grid"><div class="input-group"><label for="manageSetBrand">Marke</label><input class="input" id="manageSetBrand" name="brand" value="${esc(set.brand)}"></div><div class="input-group"><label for="manageSetLine">Produktlinie</label><input class="input" id="manageSetLine" name="line" value="${esc(set.line)}"></div><div class="input-group"><label for="manageSetSize">Größe</label><input class="input" id="manageSetSize" name="size" value="${esc(set.size)}" required></div><div class="input-group"><label for="manageSetUse">Verbrauch / Tag</label><input class="input" id="manageSetUse" name="dailyUse" type="number" min="0" max="20" step="0.1" value="${set.dailyUse}" required></div></div>${existing?'':`<div class="input-group"><label for="manageSetStock">Startbestand</label><input class="input" id="manageSetStock" name="stock" type="number" min="0" value="${stock}" required></div>`}<button class="btn primary block">Set speichern</button></form>`);
  }
  ...
  document.addEventListener('submit',(e)=>{
    ...
    if(e.target.id==='addStockForm') { e.preventDefault(); const f=new FormData(e.target); repository.addStock(e.target.dataset.childId,e.target.dataset.setId,+f.get('amount'),{acquiredAt:f.get('acquiredAt'),note:f.get('note')}); closeModal(); toast('Vorrat ergänzt.'); render(); }
    if(e.target.id==='setForm'){e.preventDefault();const f=new FormData(e.target),childId=e.target.dataset.childId,input={label:f.get('label'),brand:f.get('brand'),line:f.get('line'),size:f.get('size'),dailyUse:+f.get('dailyUse')};if(e.target.dataset.id)repository.updateSet(childId,e.target.dataset.id,input);else repository.createSet(childId,{...input,stock:+f.get('stock')});family.clearSelection();closeModal();toast('Windelset gespeichert.');render();}
    if(e.target.id==='lotForm'){e.preventDefault();const f=new FormData(e.target);repository.updateLot(e.target.dataset.childId,e.target.dataset.setId,e.target.dataset.id,{initialUnits:+f.get('initialUnits'),remainingUnits:+f.get('remainingUnits'),acquiredAt:f.get('acquiredAt'),note:f.get('note')});closeModal();toast('Vorratsposten gespeichert.');render();}
    if(e.target.id==='barcodeForm'){e.preventDefault();const f=new FormData(e.target);barcodeResult(f.get('barcode'));}
    if(e.target.id==='correctionForm'){e.preventDefault();const f=new FormData(e.target);MyDiaper.catalogCorrectionRepository.save({barcode:e.target.dataset.barcode,kind:f.get('kind'),note:f.get('note')});closeModal();toast('Lokaler Korrekturentwurf gespeichert.');render();}
    if(e.target.id==='barcodeStockForm'){e.preventDefault();repository.assignProduct(e.target.dataset.childId,e.target.dataset.setId,e.target.dataset.productSizeId);repository.addStock(e.target.dataset.childId,e.target.dataset.setId,+e.target.dataset.units,{note:'Per Barcode erfasst'});closeModal();toast('Produkt und Vorrat übernommen.');render();}
    if(e.target.id==='reminderForm'){e.preventDefault();const f=new FormData(e.target),config=MyDiaper.domain.reminders.normalize({stockDays:+f.get('stockDays'),sizeCheckWeeks:+f.get('sizeCheckWeeks'),quietStart:f.get('quietStart'),quietEnd:f.get('quietEnd')});MyDiaper.store.patch(state=>state.settings.reminderConfig=config);closeModal();toast('Erinnerungen gespeichert.');render();}
    ...
    if(e.target.id==='marketSearchForm'){e.preventDefault();ui.marketQuery=String(new FormData(e.target).get('query')||'').trim();closeModal();render();}
    if(e.target.id==='marketReportForm'){e.preventDefault();const f=new FormData(e.target);MyDiaper.marketplaceRepository.report(e.target.dataset.id,{reason:f.get('reason'),note:f.get('note')});closeModal();toast('Anzeige lokal markiert.');render();}
    if(e.target.id==='listingForm') { e.preventDefault(); const f=new FormData(e.target);MyDiaper.marketplaceRepository.save({id:e.target.dataset.id||undefined,mode:f.get('mode'),title:f.get('title'),description:f.get('description'),condition:f.get('condition'),count:+f.get('count'),priceText:f.get('price')||undefined,delivery:f.get('delivery'),region:f.get('region')}); closeModal(); toast('Anzeige lokal gespeichert.'); render(); }
