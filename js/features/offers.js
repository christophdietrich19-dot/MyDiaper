(function(){
  MyDiaper.features.offers=function(ctx){
    ...
    const mapped=offers.filter(offer=>MyDiaper.offerMap?.coordinates(offer));
    ...
    return `<div class="nearby-screen">${ctx.screenHeader(local?'Angebote in deiner Nähe':'Online-Angebote','today','offer-search')}
      ...
      ${local&&!list?`<div class="real-map-wrap"><div id="offerMap" class="nearby-map real-map" role="application" aria-label="Interaktive Karte${mapped.length?` mit ${mapped.length} verorteten Angeboten`:''}"></div><span class="demo-map-label">Echte Karte · ${mapped.length?`${mapped.length} verortete${mapped.length===1?'s':''} Angebot${mapped.length===1?'':'e'}`:'keine erfundenen Händlerpins'}</span><button class="map-location-button" data-action="edit-location">${ctx.icon('pin')} Standort / Ort</button></div>`:''}
      ...
      <div class="offer-data-note"><strong>Datenquellen:</strong> Karte © OpenStreetMap-Mitwirkende · ${providers.map(provider=>ctx.esc(provider.label)).join(' · ')}. Nur Angebote mit gelieferten Koordinaten erscheinen als Marker; Listenangebote werden nicht künstlich auf der Karte verteilt.</div>
