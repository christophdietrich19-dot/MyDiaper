(function(root){
  ...
  function createOfferRepository(store,catalog){
    ...
    function validateTarget(state,childId,setId,productSizeId){
    ...
    function saveAlert(childId,input){
      ...
      store.patch(state=>{
        validateTarget(state,childId,input.setId||null,normalized.productSizeId);
        ...
        if(!alert){
          alert=state.priceAlerts.find(item=>item.childId===childId&&item.productSizeId===normalized.productSizeId&&item.scope===normalized.scope)||
