...
Testversion 1.1.4 enthält weiterhin die Darstellungsreparaturen aus 1.1.2: Das Babybild besitzt nun echte PNG-Transparenz und bleibt aus dem Textbereich, Profilkarten erzeugen auch auf schmalen Displays keinen horizontalen Überlauf, die Bottom-Navigation berücksichtigt Android-/iOS-Safe-Areas über Capacitor SystemBars und sämtliche Erinnerungsschalter verwenden dieselbe geometrisch zentrierte Toggle-Regel.

Neu in 1.1.4: Dialoge sperren beide Dokumentebenen und stellen die vorherige Scrollposition wieder her. Identische Hinweise werden zusammengeführt; gleichzeitig erscheinen höchstens drei unterschiedliche Toasts. Die sichtbare Versionsangabe wird aus `package.json` in Web-, Android- und iOS-Metadaten synchronisiert. Die Angebotskarte verwendet keine erfundenen Berliner Ersatzkoordinaten mehr: echte Gerätepositionen richten nur die Karte aus, und Händlerpins erscheinen ausschließlich für Angebote, die vollständige Koordinaten aus einer kontrollierten Quelle mitbringen.
...
Die Startseite zeigt tatsächlichen erfassten Verbrauch und Vorrat; Windelwechsel können als nass, Stuhlgang, beides oder trocken protokolliert, bearbeitet und rückgängig gemacht werden. Tages- und Sieben-Tage-Zusammenfassungen bleiben pro Kind getrennt. Die Angebote verwenden nun eine interaktive OpenStreetMap-Karte über Leaflet. Die Karte ist echt, die Händlerpositionen, Entfernungen und Preise sind weiterhin deutlich markierte Demo-Daten. Der Gerätestandort wird ausschließlich nach einem Tipp einmalig gelesen, nicht exakt gespeichert und nie im Hintergrund verfolgt; Ort oder PLZ lassen sich weiterhin manuell speichern. Zusätzlich lassen sich eigene, kontrollierte JSON-Angebotsquellen lokal einlesen.

Beim ersten Start kann gewählt werden, ob die Begrüßung „Mama“, „Papa“, neutral, mit Name/Spitzname oder ganz ohne persönliche Anrede erfolgt. Android-Zurück schließt zuerst Dialoge, geht im Finder schrittweise zurück, führt anschließend zur Startseite und beendet die App erst nach zweimaligem Drücken. Offene Dialoge sperren den Hintergrund-Scroll und warnen vor dem Verwerfen geänderter Formulare. Seitenzoom ist nur in der Android-App deaktiviert; Browser-Zoom, Bedienungshilfen und Karten-Gesten bleiben erhalten.
...
- wählbare Begrüßung beim ersten Start und später im Profil
...
- Erfahrungseditor für Passform, Auslaufschutz, Nachtleistung, Hautkomfort, Preis-Leistung, Größenwirkung und Wiederkauf
...
- einzelne Vorratsposten mit damaligem Produkt, Lagerort, Datum und Notiz vollständig verwalten
- Windelwechsel-Protokoll mit Inhalt, Korrektur, Rückgängig sowie Tages-/Wochenübersicht
- Lokale und Online-Angebote klar getrennt
- Preis-pro-Windel-Berechnung
- austauschbarer OfferProvider und kindbezogene lokale Preisalarme für Stückpreis und/oder exakte Packung
- interaktive Leaflet-/OpenStreetMap-Karte mit einmaliger, freiwilliger Standortausrichtung
...
- `js/domain/` – unabhängige Regeln für Altersstufen, Aktivitäten, Katalog, Barcodes, Angebote, Börse, Reminder, Backup, Größen, Fit-Check, Personalisierung, Vorrat und Stückpreise; Modellmigration
...
- `js/platform/` – Browserdateien, einmaliger Standort und Android-Navigation hinter Web/Capacitor-Grenzen
...
- `js/ui/product-fields.js` – strukturierte Produktauswahl mit Suche und manueller Eingabe
- `js/ui/offer-map.js` – Leaflet-Karte mit OSM-Kacheln und Demo-Markern
...
Die Auswahl unter **Windeln → Aktives Set** bestimmt das jeweilige Set. Vorrat, Windelwechsel, Fit-Check, Verlauf und Produkterfahrung beziehen sich anschließend genau auf dieses Set. Die Auswahl bleibt pro Kind während der Sitzung erhalten; beim nächsten Start wird das Hauptset verwendet. Unter **Set bearbeiten** lassen sich Produkt, Größe, Gesamtbestand und Verbrauch anpassen. **Sets verwalten** deckt den kompletten lokalen Lifecycle ab; **Vorratsposten verwalten** bearbeitet einzelne Packungen samt Lagerort getrennt. Ein späterer Produktwechsel am Set ändert vorhandene Vorratsposten nicht rückwirkend.
...
Beim Öffnen am bisherigen Datei-/Web-Speicherort migriert die App `mydiaper-v1-state`, `mydiaper-v2-state`, `mydiaper-v3-state` oder `mydiaper-v4-state` einmalig nach `mydiaper-v5-state`. Die vorherigen Schlüssel bleiben unverändert als Sicherung bestehen. Alte Verbrauchsereignisse erhalten den ehrlichen Inhalt „nicht erfasst“, vorhandene Vorratsposten den Lagerort „Zuhause“, und die neue Anrede bleibt bis zur Auswahl unbestätigt. Die übrigen Profil-, Einstellungs-, Anzeigen- und Chatdaten werden übernommen.
...
Zusätzliche Tests prüfen den vierstufigen Finder ohne unbeabsichtigte Profiländerungen, kindgetrennte Entwürfe, Set-Zuordnung, persönliche Signale, Prioritätshinweise, Katalog- und Providerregeln, v1/v2/v3/v4→v5-Migration, Kinder-/Set-/Vorrats-CRUD, Vorratsorte, Windelwechsel samt Rückgängig, Reminder, Barcodes, lokale Börsensicherheit, Produkt-/Größenverläufe, beide Preisalarm-Arten, kontrollierte Angebotsimporte, Backup-Rundläufe, Verwaltungsansichten und Offline-Assets.
